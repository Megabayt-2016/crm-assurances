...
<a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{url('/dashboard')}}">
    <div class="sidebar-brand-icon ">
      <i>
         <img src="{{asset('../img/CCA-removebg-preview.png')}}" style="margin-top: 15%; width:65%; margin-left:15%"/>
       </i>
    </div>
 </a>
                    ...
                    <div x-data="app()" x-init="[initDate(), getNoOfDays()]" x-cloak>
						...
						<div style=" background-color: rgba(0, 0, 0, 0.8)" class="fixed z-40 top-0 right-0 left-0 bottom-0 h-full w-full" x-show.transition.opacity="openEventModal">
							<div class="p-4 max-w-xl mx-auto relative absolute left-0 right-0 overflow-hidden mt-24">
								...
								<form class="form" id="form" action="{{url('Admin/calendrier')}}" method="POST">
										@csrf
									<div class="mb-4">
    ...
    <script>
		...
		function app() {
			return {
               ...
               try{
                  const fetchRes = await fetch("{{ route('admin.calendrier.show') }}", {
                     method: 'GET', 
                     headers: {
               ...
               try{
                  const fetchRes = await fetch("{{ route('admin.calendrier.store') }}", {
                     method: 'POST', 
                     headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
