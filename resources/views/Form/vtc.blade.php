            ...
            <div class="card mb-4">
                <div class="card-body text-center bg-info">Bonjour
                    <strong>{{ \App\Models\Client::find(decrypt($client_id))->nom}} {{ \App\Models\Client::find(decrypt($client_id))->prenom}}</strong><span
                        class="text-end">(Auto. VTC Risque Professionnel)</span></div>
            </div>
            <form method="post"
                  action="{{ route('uploadingvtc', ['client_id' => request()->segment(count(request()->segments()))]) }}"
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-md-4 mb-4">
                        <label for="carteProfesionnelleRecto" class="form-label">Carte Profesionnelle (recto)</label>
                        <input class="form-control @error('carteProfesionnelleRecto') is-invalid @enderror" type="file"
                               name="carteProfesionnelleRecto" id="carteProfesionnelleRecto"/>
                        @error("carteProfesionnelleRecto")
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>

                    <div class="col-md-4 mb-4">
                        <label for="carteProfesionnelleVerso" class="form-label">Carte Profesionnelle (verso)</label>
                        <input class="form-control @error('carteProfesionnelleVerso') is-invalid @enderror" type="file" name="carteProfesionnelleVerso" id="carteProfesionnelleVerso"/>
                        @error("carteProfesionnelleVerso")
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-4">
                        <label for="kBis" class="form-label">K-Bis</label>
                        <input class="form-control @error('kBis') is-invalid @enderror" type="file" name="kBis" id="kBis"/>
                        @error("kBis")
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
